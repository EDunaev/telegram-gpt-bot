...
- Групповой чат с `chat.id == CHAT_ID` и типом `group`/`supergroup` → разрешено, если в тексте есть
  упоминание `BOT_USERNAME`, либо сообщение — reply на сообщение бота, **либо reply на сообщение
  с картинкой от кого угодно** (см. ниже, зачем нужен третий пункт).
- Всё остальное — запрещено. В частности, ADMINS не даёт автодоступа в группе — это правило только
  для приватных чатов; в группе даже админ обязан упомянуть бота или ответить на его сообщение
  (или на фото), иначе `is_allowed` тихо вернёт `False` без единой строчки лога от хендлера (в
  `bot.log` останется только строка из самого `is_allowed`, дальше — тишина).
...
Этого одного было недостаточно: `is_allowed` в группе разрешает доступ только по упоминанию бота
или reply **на сообщение бота** — reply на фото от обычного пользователя без упоминания бота
блокировался ещё до того, как код добирался до `handle_text` (полная тишина в `bot.log`, ни одной
строки от хендлера). Поэтому `is_allowed` дополнительно разрешает reply на сообщение с картинкой
от кого угодно (`_extract_image_source(replied) is not None`) — иначе фича "спроси про фото по
reply" не работает именно в группе, где нет мемори и нет обязательного упоминания при каждом
сообщении.
